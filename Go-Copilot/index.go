...
func login(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := db.Raw("SELECT name, count(*) FROM user_models WHERE email = ? AND password = ? GROUP BY name", &user.Email, &user.Password).Rows() // (*sql.Rows, error)
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()
...
func createUser(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := db.Raw("SELECT count(*) FROM user_models WHERE email = ?", &person.Email).Rows() // (*sql.Rows, error)
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()
...
func main() {
	...
	myRouter.HandleFunc("/page={page}", allUser).Methods("GET")
	...
	myRouter.HandleFunc("/target/{iddd}", getTarget).Methods("GET")
